	...
	var BtValidate=require("../util/validate.js"),
		...
		$("#invoice-info").delegate('#invoice-radio-new','change',function(){   
			...
			if($(this).prop("checked")){   
				...
				$("#invoice-form-box").html(template("invoice-form-template",{list:invoiceInfoarry[$(this).attr("data-index")]}));
			}
		});		
		...
		$("#invoice-info").delegate(".invoice-editor",'click',function(){
			...
			}else if($("[name=invoice]:checked").length>0){  
		...
		}).delegate("[name=address]","change",function(){
			if($(this).prop("checked")){     
				var thisAddress=consigneesList[$(this).attr("data-index")]  
				var tel=thisAddress.tel;    
				if(tel){
					var telarray = tel.split("-");
					thisAddress.qh=telarray[0];
				...
				$("#address-form-box").html(template("address-form-template",{list:thisAddress}));  
				areafun();
				$(".address-item").removeClass("address-item-current");
				$(this).parent().addClass("address-item-current");  
			}
		});     
